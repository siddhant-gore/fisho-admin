...
import { FaCheck, FaFileInvoice, FaLocationDot } from "react-icons/fa6";
...
const ViewOrder = () => {
  ...
  const [updateBulkOrder, { isLoading: updateLoading }] =
    useUpdateBulkOrderByIdMutation();
  ...
  const orderBtn = (record) => {
    ...
    const colorClass =
      statusColorMap[mappedStatus] || "text-gray-600 bg-gray-100";
    const label = formatStatusLabel(mappedStatus);

    return (
      <span
        className={`px-2 py-1 rounded-full text-sm  text-nowrap font-medium ${colorClass}`}
      >
        {label}
      </span>
    );
    ...
    return (
      ...
      >
        <div
          ...
          style={{
            color: OrderStatuses?.find((s) => s.key === currentKey)?.color,
          }}
        >
          {OrderStatuses?.find((s) => s.key === currentKey)?.label ||
            "Select Status"}
        </div>
      </Dropdown>
    );
        ...
        {order ? (
            ...
            title={
              <div className="flex justify-between">
                ...
                {order?.invoice && (
                  <Button
                    className=" "
                    onClick={() => handleDownloadReciept(order?.invoice)}
                  >
                    Download Invoice
                    <FaFileInvoice
          ...
          >
            ...
            <p className="my-0 text-center">
              Floor Number {order?.address?.address?.flatNumber}
            </p>
            <p className="my-0 text-center">
              Flat/Villa Number: {order?.address?.address?.houseNumber}
            </p>
            <p className="my-0 text-center">
              Building Name: {order?.address?.address?.houseName}
            </p>
            <p className="my-0 text-center">
            ...
            <div className="flex justify-around mb-4  ">
              ...
              <div>
                <p className=" ">
                  ...
                  <span
                    className={`${
                      order?.paymentMade ? "text-green-600" : "text-red-600"
                    }`}
                  >
                    {order?.paymentMade ? "Paid" : "Not Paid"}
                  </span>
                </p>
                <p className="">
                  ...
                  <DirhamSymbol /> {order?.earnings}
                </p>
                <p className="">
                  <span className="font-semibold">Delivery cost:</span>{" "}
                  <DirhamSymbol /> {order?.delivery_cost}
                </p>
                <p className="">
                  <span className="font-semibold">Delivery By:</span>{" "}
                  {order?.deliveryBy
                    ? new Date(order?.deliveryBy).toLocaleString()
                    : "-"}
                </p>
              </div>
            </div>
            ...
            {order?.special_req && (
              <p className="mt-2 font-semibold">
                Special Request:
                <span className="border rounded-md ms-2 px-2">
                  {order?.special_req}
                </span>
              </p>
            )}
            ...
            <div className="text-center">
                <Button
                  loading={loading}
                  disabled={order?.readytoPickUp}
                  onClick={handleFireOrder}
                  className={`bg-green-500 mt-2 font-semibold`}
                >
                ...
                <p className="mt-2 font-semibold">
                  Preferred Time Slot:
                  <span className="border rounded-md ms-2 px-2 font-bold" >
                    {order?.slot ?? 'N/A'}
                  </span>
                </p>
